...
public class ScoreCalculator {
	...
	public void process(){
		...
		for(int i=0;i<inpFiles.length;i++){
			System.out.println("calculating TF/IDF of "+inpFiles[i]);
		...
		try {
			...
			for(int i=0;i<dataWord.length;i++){
				...
				if(wordSet.containsKey(dataWord[i])){
					if(wordSet.get(dataWord[i]).containsKey(actualFileName)){
						wordSet.get(dataWord[i]).put(actualFileName, wordSet.get(dataWord[i]).get(actualFileName)+1);
						/*temp=wordSet.get(dataWord[i]);
						int freq = temp.get(actualFileName);
						freq++;
						temp.put(actualFileName, freq);
						wordSet.put(dataWord[i], temp);
						if("ac".equals(dataWord[i])){
							System.out.println("ac-----"+freq);
						}*/
					}else{
