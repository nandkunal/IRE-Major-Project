...
import org.ire.uima.tokenizer.CapitalAnnotator.Maker;
...
public class POSAnnotator extends JCasAnnotator_ImplBase {
	...
	void makeAnnotations(Maker m, BreakIterator b) {
		...
		for (int end = b.next(), start = b.first(); end != BreakIterator.DONE; start = end, end = b
			...
			if (!isWhitespace) {
				if (PrintAnnotations.positiveSet.contains(input.substring(start, end))) {
					m.newAnnotation(jcas, start, end).addToIndexes();
				}else if (PrintAnnotations.negativeSet.contains(input.substring(start, end))) {
					m.newAnnotation(jcas, start, end).addToIndexes();
				}
