 ...
 */
public class DocCountAnnotator extends JCasAnnotator_ImplBase {
	...
	static final Maker tokenAnnotationMaker = new Maker() {
		Annotation newAnnotation(JCas jcas, int start, int end) {
			return new DocCount(jcas, start, end);
		}
	};
	...
	void makeAnnotations(Maker m, BreakIterator b) {
		b.setText(input);
		String patternString = "^\\s+[A-Za-z,;'\"\\s]+[.?!]$";
		...
		while(matcher.find(stIndex)){
			System.out.println(input.substring(matcher.start(), matcher.end()));
			m.newAnnotation(jcas, matcher.start(), matcher.end()).addToIndexes();
			String inp[] = input.substring(matcher.start(), matcher.end()).split("\\s");
			stIndex=inp[0].length()+matcher.start();
